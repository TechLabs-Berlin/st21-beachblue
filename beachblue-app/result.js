...
function cardFill(json_beach){
  ...
  for (let i = 0; i < 6; i++) {
    $("#cards-dynamic").append(
      `<div class="result-card" id = "beach-id-`+ json_beach[i]["Unnamed: 0"]+ `"> 
        <img class="result-card-img" src="./styles/img/beach-1.png" alt=""> 
        <div class="card-body"> 
      ...
      for (var [key, value] of Object.entries(json_beach[i])) {
        ...
        if (key == 'Condition'){
          $("#beach-id-" + json_beach[i]["Unnamed: 0"]).find('.tags').append(`<a class="tag" href="">`+ value +`</a>`)
        }
