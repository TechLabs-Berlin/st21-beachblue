...
df['Sea_Promenade_1'] = (df['Sea_Promenade'].replace(mapping))*5

df['Nudism_1'] = (df['Nudism'].replace(mapping))*4

df['Blue_Flag_1'] = (df['Blue_Flag'].replace(mapping))*4

df['Reachable_by_bus_1'] = (df['Reachable_by_bus'].replace(mapping))*4

df['Parking_1'] = (df['Parking'].replace(mapping))*3

df['Toilets_1'] = (df['Toilets'].replace(mapping))*5

df['Footbaths_1'] = (df['Footbaths'].replace(mapping))*5

df['Showers_1'] = (df['Showers'].replace(mapping))*5

df['Public_Telephones_1'] = (df['Public_Telephones'].replace(mapping))*2

df['Trashcans_1'] = (df['Trashcans'].replace(mapping))*1

df['Child_Zone_1'] = (df['Child_Zone'].replace(mapping))*4

df['Sport_Zone_1'] = (df['Sport_Zone'].replace(mapping))*5

df['Yacht_Club_1'] = (df['Yacht_Club'].replace(mapping))*5

df['Surfing_Zone_1'] = (df['Surfing_Zone'].replace(mapping))*5

df['quality2019_1'] = (df['quality2019']).replace(quality)*5

df['Sum'] = df[['Sea_Promenade_1','Nudism_1','Blue_Flag_1','Reachable_by_bus_1','Parking_1','Toilets_1','Footbaths_1','Public_Telephones_1','Trashcans_1','Child_Zone_1','Sport_Zone_1','Yacht_Club_1','Surfing_Zone_1','quality2019_1']].sum(axis=1)
...
print(df[['ratings_1','ratings_2','Sum','rating']].head())
...
data = df[['Sea_Promenade_1','Nudism_1','Blue_Flag_1','Reachable_by_bus_1','Parking_1','Toilets_1','Footbaths_1','Public_Telephones_1','Trashcans_1','Child_Zone_1','Sport_Zone_1','Yacht_Club_1','Surfing_Zone_1']]
